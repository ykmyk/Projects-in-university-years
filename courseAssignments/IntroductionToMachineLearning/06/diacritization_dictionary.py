...
def diacritize_sentence(model,
                        dictionary: Dictionary,
                        train_singletons: dict[str, str],  # NEW
                        nd_sentence: str,
                        window: int) -> str:
    """
    Fast per-word diacritization:
      - For each word:
        ...
        * If in dictionary with multiple variants: call predict_proba once per
          relevant character and score all variants using cached probs.
        * If not in dictionary and not in train_singletons: char-wise fallback using predict_proba.
    """
    ...
    while i < n:
        ...
        # Case 1: dictionary has exactly one variant → no model calls
        ...
        # Case 2: word is in dictionary with multiple variants → choose best variant
        if variants:
            best_score = -np.inf
            best_variant = None
...
def main(args: argparse.Namespace) -> Optional[str]:
    if args.predict is None:
        ...
        # --- NEW: build train_singletons for words NOT in dictionary ---
        dictionary = Dictionary()
        singleton_tmp: dict[str, Optional[str]] = {}
        train_nodia_words = train.data.split()
        train_dia_words = train.target.split()

        for nd, dia in zip(train_nodia_words, train_dia_words):
            key = nd.lower()
            # skip words already in the provided dictionary
        ...
        train_singletons = {
            k: v for k, v in singleton_tmp.items() if v is not None
        }
        ...
        model = sklearn.pipeline.Pipeline([
            ("vec", TfidfVectorizer(analyzer="char", ngram_range=(1, 5), max_features=0000)),
            ("clf", sklearn.linear_model.LogisticRegression(multi_class="multinomial", solver="saga",
                                                            C=5.0, max_iter=500, n_jobs=-1)),
        ])
        ...
        # NEW: save bundle (model + train_singletons)
        bundle = {"model": model, "train_singletons": train_singletons}
        ...
        with open(args.predict, "r", encoding="utf-8") as f:
            for line in f:
                ...
                sentence = diacritize_sentence(model, dictionary, train_singletons, nd_sentence, window)
